...
| ~~Home-brand thread check~~ — **DECLINED, will not be ported** (Muxin, 2026-09-05): the scorer's sibling, and declined on the same grounds. Same origin (2026-07-04, the `/atomize` CLI), same shape (an advisory verdict written to the unread `notes` cell), same sole machine consumer (a `console.log` at `validate.ts:312`). Muxin: "If we retired scorer I don't know why we'd keep its sibling." `/atomize` keeps it as it does today. | `src/atomize/thread-check.ts` |
| `npm run validate` (char/word limits, routing gate, skeleton gate, case gate) | `src/atomize/validate.ts` |
| Per-platform quote-card **post text** (the body of the post that carries the card image; renamed from "context captions" at Muxin's direction, 2026-09-05 — "captions" wrongly read as visual subtext under the image) | `/atomize` step 7 |
| ~~Strategy brief directives (`from_brief`, `directives_applied`)~~ — **DECLINED, will not be ported** (Muxin, 2026-09-05): superseded by the Grow experiment chain. Two parallel learn-and-grade loops exist. The old one stamps a derivative with "the brief said lean into this pillar" and grades it as a bet; the new one (`src/grow/`) states a hypothesis, a controlled variable and held constants, ships as an experiment (`experiment_id`, recorded into the Placed log by 5F), and grades that. Nothing converts brief recommendations into experiment recommendations, so they duplicate rather than feed each other, and the newer chain is strictly more informative. **The brief itself stays** — `readSignals` (`src/review/signals.ts:74`) parses the latest brief for the Signals page's channel-confidence table and DO MORE/TEST/DO LESS list, and the page's "Refresh brief" runs a real `/strategy` (`serve.ts:663`). Only the stamp-and-grade mechanism hanging off it is obsolete. `/atomize` keeps directives as it does today. | `/atomize` step 2 |
