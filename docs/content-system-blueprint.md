...
| Platform/pool matrix | `src/patterns/platform-pool-matrix.ts` and `patterns:platform-pool-matrix` group only explicit platform, niche/broad/format-pool, medium/format, collection, review, and baseline facts; missing labels remain visible as gaps | Reviewed platform × pool × format coverage with explicit account examples and no inferred best creator | Scaffolded |
