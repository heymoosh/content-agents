...
| `survey` | `question` (free text, fixed to "What are you stuck on right now?" unless overridden with a recorded reason), `destination` (`confirmation_page` \| `welcome_email` \| `both`), `respondent_field_ref` |
...
| `survey` | `manual` | false | confirmation page (primary), linked from the welcome email (secondary) — corrected 2026-08-07, was "embedded in the welcome email"; matches the proposed canonical path in `venture/rules.md` §6.5/§6.8 | `url` | 2 |
