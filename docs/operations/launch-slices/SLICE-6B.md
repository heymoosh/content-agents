...
- [ ] A1: Present-source status established for each historical item with exact citations.
- [ ] A2: Queue user-visible ledger location agrees with the actual configured destination;
      actual isolated output proves it; no production scheduler changes.
- [ ] A3: Render test observes real child-side log output through the shared isolated log;
      free-render outcome remains cost-free and child/process/temp cleanup is bounded.
- [ ] A4: Actual affected tests pass and material findings receive independent closure.
- [ ] A5: Frozen final runtime gate, preservation, hygiene and coordinator integration pass.
...
Pending. `bash scripts/repo-hygiene.sh --rescue`; record exact preserved leftovers, scoped checks,
independent closure and frozen full gate before acceptance. Only coordinator commits; no push.
