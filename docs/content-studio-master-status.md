...
| Grow lifecycle | Connect source, claim/cut, variant, review, capacity, delivery, outcome, and Venture without auto-approval. | Many typed, side-effect-free records and reconciliation projections exist. **Phase 3 vertical slice is in Muxin review:** `grow-experiment-proposal-v1` binds one real approved cut and a controlled same-platform treatment pair to one digest, while composing the canonical capacity manifest, per-variant review bundles, no-winner experiment record, and approved-only delivery records. An edited body requires a fresh validated proposal; no record claims a scheduler slot or publishes. | Focused proposal/decision/CLI and retained-artifact sync tests plus the full Grow suite; the real review packet is retained under `docs/reviews/`. | Muxin must decide the real packet before the slice can be called complete. Then bind approved delivery records to the live review queue and scheduler/provider facts. Comments, funnel events, and business outcomes remain later evidence. |
