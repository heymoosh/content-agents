...
| Capability | Latest decision | Current state | Verification | Remaining work |
|---|---|---|---|---|
| Analytics and strategy | Keep attention, conversation, audience, and business separate; thin data stays insufficient; no silent routing changes. | Analytics DB, strategy briefs, bets, routing/resonance, and recommendation layers exist. The Signals brand selector scopes measurements, Strategy recommendations, decisions, and experiments to Human Inference, Charles, or Fiction; legacy unassigned records stay visibly excluded rather than being attributed silently. | Extensive deterministic query, route, UI, and browser brand-switch coverage plus historic operational data. | Landing, opt-in, and business-outcome ingestion remains incomplete. Per-brand operational proof is tracked separately below; the deterministic Studio read is partitioned. |
