...
| Cross-system learning and Venture handoff | Signals may offer a measured, qualified learning input to one named Venture; Muxin decides in Signals, then decides again at the Venture boundary. Generic engagement never becomes demand evidence. | **Phase 4 deterministic implementation complete on the current branch.** Only an accepted experiment interpretation whose exact treatment and control primary measurements both come from attributed funnel/business outcome-ledger facts can create a body-free proposal. Attention, conversation, provider-only clicks/follows, and unrelated outcome refs fail closed. Signals persists adopt/decline/request-more-evidence separately. An adopted proposal appears inside the named Venture's normal room, where a separate Venture-owned action may accept, reject, or request more evidence. Acceptance creates one internal, non-publishable `signals-input` artifact plus an append-only canon decision; it never clears a checkpoint, advances a phase, or publishes. Exact retries and concurrent accepts are idempotent; interrupted artifact-before-canon writes recover without duplication; identity drift and ledger corruption fail closed. | Red-green unit, route, emitted-browser-script, strict-client-allowlist, exact-lineage, per-arm outcome provenance, tamper, crash-recovery, cross-process concurrency, and rules-parity coverage. Final `npm run check` passes 3,756/3,756 tests, deterministic browser E2E passes 40/40 with 17 explicitly blocked live/non-deterministic paths, and the authorized Grok 4.5 read-only cross-family audit reports no release blockers and a safe review-held draft-PR verdict. | Run one real measured experiment through the complete two-room decision flow after a qualifying attributed funnel or business outcome exists. Until then, do not claim an operational Venture handoff. |
