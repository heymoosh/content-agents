...
ideas are not automatic work: Signals should withhold them unless the uncertainty is unusually
valuable, the test is cheap, and it explicitly explains why the expected learning justifies the
capacity. A proposal may honestly return no experiment. Confidence is a prioritization input, never
a claim that a treatment has already won.
...
| Experiment lifecycle | Signals ranks and proposes content-growth experiments; Muxin approves plans in Signals; Experiment creates tagged work through the canonical Content generator; Content remains the sole copy-review and publishing surface; Signals later interprets grouped outcomes. Venture owns venture-learning experiments and surveys within one venture's hypothesis chain. | **Phase 3 implementation complete.** The science packet, ranked body-free plan review, canonical Content generation handoff, concurrent experiment identities, and pending-copy boundary remain intact. The measurement loop now matches live/delivered provider identities exactly to the latest analytics row, joins explicitly attributed funnel/business facts from `data/outcomes.jsonl`, and shows each experiment as collecting or ready in Signals. Ready requires treatment and comparison facts, every guardrail, the declared sample, and elapsed duration. The selected signed-in Studio CLI model interprets the body-free measured packet; its keep/revise/reject recommendation is append-only and remains pending until Muxin accepts or rejects the analysis. No path selects a winner or changes routing automatically. | Red-green focused coverage for exact provider matching, latest analytics selection, unsupported-metric abstention, outcome-ledger attribution, two-arm/guardrail/sample/duration readiness, body-free science parsing, invented-evidence refusal, retry-safe append-only interpretation, separate Muxin review, routes, and Signals UI. Full local and browser verification evidence belongs with the shipping commit. | Run the first real approved experiment through publication and data collection. Metrics absent from provider exports, including substantive-reply classification and canonical essay visits, remain honestly collecting until explicit attributed rows enter `data/outcomes.jsonl`. Multi-pair operational cadence remains bounded by normal Content capacity and human review. |
