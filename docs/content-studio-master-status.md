...
| Draft, edit, lock | Direction input, engine choice, direct edit, revise with model, validation, immutable lock. | Implemented. The post-generation boundary now deterministically rejects an enumerated syntactic class of population/prevalence claims unless the complete normalized sentence was present in cited evidence or Muxin's direction; full-sentence authorization prevents predicate and polarity substitution. This intentionally does not pretend to prove arbitrary entailment, which remains part of pending human review. Successful execution logs retain the selected engine. | Unit/route/UI coverage plus disposable Chromium through the real first-draft and same-file revision controls, with a one-run token-gated injected engine, a real pending queue row, durable message bytes, and no live model call. A bounded authenticated Claude canary exposed the unsupported claim “That's rare. Most teams build first”; after the guard was added, an exact rerun rejected those two phrases before creating a message directory, queue row, or cost record. This verifies authenticated adapter execution and fail-closed refusal, not acceptable live draft quality. | Authenticated acceptable model output remains operationally unverified. Repeat the bounded canary until a grounded draft passes the guard and human review; do not weaken the guard merely to obtain a pass. |
