...
| Cross-system learning and Venture handoff | Signals may offer a measured, qualified learning input to one named Venture; Muxin decides in Signals, then decides again at the Venture boundary. Generic engagement never becomes demand evidence. | **Phase 4 deterministic implementation complete on the current branch, including the 2026-09-01 audit corrections.** Only an accepted experiment interpretation whose exact treatment and control primary measurements both come from attributed funnel/business outcome-ledger facts can create a body-free proposal. Attention, conversation, provider-only clicks/follows, unrelated outcome refs, and missing Content lineage fail closed. Signals persists adopt/decline/request-more-evidence separately, and proposal events cannot forge that decision projection. Both rooms expose scope, samples, provenance, caveats, qualification, evidence status, Venture/phase, and exact lineage. An adopted proposal appears inside the named Venture's normal room, where a separate Venture-owned action may accept, reject, or request more evidence. Acceptance creates one internal, non-publishable `signals-input` artifact plus an append-only canon decision; it never clears a checkpoint, advances a phase, publishes, selects a winner, or changes configuration. Exact retries and concurrent accepts are idempotent; interrupted artifact-before-canon writes recover without duplication; identity drift and ledger corruption fail closed. | Red-green unit, full-lifecycle production-route, emitted-browser-script, strict-client-allowlist, exact-lineage, distinct per-arm outcome provenance, forged-adoption, empty-lineage, crash-recovery, cross-process concurrency, and rules-parity coverage. The bounded Grok 4.5 audit's critical/high findings are remediated in the current worktree; final local check and browser evidence are recorded in PR #420 after remediation. | Run one real measured experiment through the complete two-room decision flow after a qualifying attributed funnel or business outcome exists. Until then, do not claim an operational Venture handoff. |
