...
| Experiment lifecycle | Signals recommends and later interprets content-growth experiments; Experiment executes approved controlled tests without auto-approval. Venture owns venture-learning experiments and surveys within one venture's hypothesis chain. | The Phase 3 packet now includes a body-free Signals science request, retained raw response, strictly parsed recommendation with evidence/prompt/response digests, directional hypothesis, rationale, controls, metrics, guardrails, decision rules, caveats, and capacity rationale. Experiment fails closed if that evidence is missing or inconsistent. Both candidates passed the ordinary cold-feed editor with input/output body digests before the renamed Experiment proposal was built. The packet is pending Muxin's item-level review and makes no provider, publication, measurement, or winner claim. | Deterministic science prompt/parser, proposal/decision/export/handoff/scheduling/running-record/CLI, conflict, idempotency, lineage, failure/uncertainty, publisher timestamp, retained-packet, and Grow-suite coverage, plus a disposable Chromium pass against the local Experiment HTML. | Obtain Muxin's review of the regenerated packet. After approval, the remaining operational evidence is an explicitly authorized provider action and observed result; Signals interpretation still requires measured outcomes. Keep Venture survey learning venture-scoped and require reviewed handoffs for cross-system generalization. |
