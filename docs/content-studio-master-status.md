...
| Studio capture | One front door accepts a thought or link, identifies the destination, explains it, and starts the next safe step. | `studio-capture-v1` records are repository-owned under the operational data root, locked, idempotent, and restart-safe. The current classifier has five actionable destinations: Content reserves and starts one durable advisor job; Fiction, Outreach, Venture, and Charles open the existing room-owned human gate with the saved capture visible. Charles routing requires the explicit persona name, copies the exact capture into empty Charles Input, and never clicks Draft; a different unsaved Charles idea is preserved and blocks the copy truthfully. Unknown destinations fail closed instead of falling through to Venture. It does not claim to classify directly to Signals. | Deterministic cross-process, crash-recovery, route, classifier-mirror, exhaustive-dispatch, and UI-contract coverage for those five destinations. Disposable Chromium proves both the Content advisor start and the Charles exact-input/no-job/no-overwrite handoff through the real Studio controls. | Add direct Signals classification only after Signals has a room-owned safe capture action; until then the global bar is not evidence of direct seven-room classification. |
...
| Venture Studio UI | Work/Documents/Intake and guardrails/History, decision and artifact actions, response intake, evidence, pace/checkpoint, one engine-owned next step, and queued per-artifact delivery/retry controls. | Implemented across Venture review modules; delivery reuses the existing manual handoff and shared Substack slot machinery, while retry is exposed only for provider failures classified retryable. | Deterministic UI, lifecycle, queue, and browser write coverage. | Real model step and authenticated Substack delivery remain provider-unverified. |
...
| Draft, edit, lock | Direction input, engine choice, direct edit, revise with model, validation, immutable lock. | Implemented. The prompt now directs the model to open with the evidenced practice instead of inventing an encounter, and the post-generation boundary deterministically rejects enumerated population/prevalence and Muxin-interest syntactic classes. Population/prevalence claims require the complete normalized sentence in cited evidence or Muxin's direction; first-person, selected-adverb, greeting-elliptical, and bare-opening read/saw/liked/loved/enjoyed/followed/worked-on claims require the complete normalized generated sentence in Muxin's direction. Full-sentence authorization prevents predicate, object, and polarity substitution. These bounded grammars intentionally do not pretend to prove arbitrary entailment, which remains part of pending human review. Successful execution logs retain the selected engine. | Unit/route/UI coverage plus disposable Chromium through the real first-draft and same-file revision controls, with a one-run token-gated injected engine, a real pending queue row, durable message bytes, and no live model call. Authenticated Claude canaries exposed “That's rare. Most teams build first,” repeated “most teams/people” variants, and invented “I saw”/“saw that” encounters; the hard guards rejected these before creating a message directory, queue row, or cost record. A later sample passed both syntactic guards but human review still rejected its unsupported presupposition that the test had already broken an assumption. This verifies authenticated adapter execution, fail-closed refusal, and the independent human gate, not acceptable live draft quality. | Authenticated acceptable model output remains operationally unverified. Repeat the bounded canary until a grounded draft passes both the guards and human review; do not weaken either gate merely to obtain a pass. |
...
| Capability | Latest decision | Current state | Verification | Remaining work |
|---|---|---|---|---|
| Persona drafting | One-liner, essay, and reply composed under `charles/config/persona.yaml`; memes stay external; never apply Muxin's voice. | Implemented. Every newly generated queue row is stamped with its selected engine, the queue is normalized to a backward-compatible engine column, and Studio shows that provenance while labeling older rows as unstamped legacy drafts. | Strong deterministic prompt, leak-bank, queue-provenance, and UI tests. A bounded authenticated Claude one-liner canary ran through the exact production adapter in a disposable repository snapshot: it created exactly one valid persona-governed draft and one `pending` queue row stamped `claude`, used the configured `calm_story_vs_frantic_maintenance` angle, immediately walked back its accidental slip, invented no factual leak, and left the shared Charles subtree unchanged after evidence capture. | Grok/Codex drafting and the essay/reply modes remain operationally unverified. Repeat a bounded authenticated canary when an engine adapter changes, and preserve the leak-bank truthfulness boundary. |
