...
| Experiment lifecycle | Signals ranks and proposes content-growth experiments; Muxin approves plans in Signals; Experiment creates tagged work through the canonical Content generator; Content remains the sole copy-review and publishing surface; Signals later interprets grouped outcomes. Venture owns venture-learning experiments and surveys within one venture's hypothesis chain. | The Phase 3 science packet and execution mechanics are implemented and Muxin approved their rationale on 2026-08-31. The corrected integration now ranks body-free plans in Signals, defers low-confidence work, requires a separate Muxin plan decision, invokes the canonical configured Content generator, and lands experiment-tagged rows `pending` in Content. Multiple plan and handoff identities coexist without a singleton lock. The retained standalone HTML is historical evidence, not a second copy-review surface. | Deterministic science prompt/parser, plan ledger and routes, confidence ordering, approved-cut revalidation, canonical generation handoff, pending-copy state, UI wiring, concurrent identities, grouped-performance read model, and focused integration coverage. The full local suite printed all 2,116 groups passing but retained its known open handle instead of exiting. | Wire real provider observations and outcome-ledger facts into the grouped-performance read, expose collecting/ready analysis in Signals, and persist a reviewed keep/revise/reject interpretation without automatic winner selection. Until that end-to-end measurement loop is proven, Phase 3 remains incomplete. |
