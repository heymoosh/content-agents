...
function main(): void {
  ...
  const out: string[] = [
    ...
    "",
    "Blocked means the feature is behind a route that spawns a model job (a `claude -p` run or a",
    "paid API). The suite refuses to trigger those, so their end-to-end behaviour is genuinely",
    "unverified rather than quietly counted as passing. Pass D lists every one of them.",
    "",
