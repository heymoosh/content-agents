...
export function renderLedgerAccountExampleTableMarkdown(value: LedgerAccountExampleTable): string {
  const lines = [
    ...
    "",
    "| Account | Handle / creator | Size | Topics | Focus | Platform | Medium | Format | Pool | Scope | Evidence | Status | Blockers |",
    "|---|---|---|---|---|---|---|---|---|---|---|---|---|",
    ...value.table.rows.map((row) => `| ${markdown(row.accountId)} | ${markdown(row.handle)} / ${markdown(row.creator)} | ${markdown(row.accountSizeSnapshot ? JSON.stringify(row.accountSizeSnapshot) : null)} | ${markdown(row.topics?.join(", "))} | ${markdown(row.focus?.join(", "))} | ${markdown(row.platform)} | ${markdown(row.medium)} | ${markdown(row.format)} | ${markdown(row.pool)} | ${markdown(`${row.popularityScope} / ${row.sampleScope}`)} | ${markdown(row.evidenceLinks.join(", "))} | ${markdown(row.readiness.status)} | ${markdown(row.readiness.blockers.join("; "))} |`),
    "",
