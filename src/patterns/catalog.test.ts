...
test("catalog keeps configured targets distinct from collected evidence and aggregates nullable facts", () => {
  const catalog = buildCatalog(config, [
  ...
  ], [
    {
      ...
      provenance: "sample was found by search",
    },
    {
  ...
  assert.deepEqual(collected.topics, ["building-solopreneur", "civic-democracy"]);
  assert.deepEqual(collected.formats, ["image", "text", "video"]);
  assert.deepEqual(collected.mediaForms, ["image", "video"]);
  assert.deepEqual(collected.popularityScopes, ["niche", "platform-wide", "search-biased"]);
  ...
  assert.match(markdown, /niche, platform-wide, search-biased/);
