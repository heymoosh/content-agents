...
export function renderCoverageMarkdown(inventory: CorpusInventory): string {
  ...
  const lines: string[] = [
    ...
    `- Entries whose metric list is partly unreadable: ${totals.entries_with_incomplete_metrics}`,
    `- Entries carrying a direct source link: ${totals.entries_with_a_source_link} of ${totals.entries}`,
    ...
    ...inventory.creators.map((creator) => [
      ...
      `${creator.metrics_available_entries}/${creator.actual_entries}${creator.metrics_incomplete_entries > 0 ? ` (${creator.metrics_incomplete_entries} partial)` : ""}`,
      `${creator.entries_with_source_link}/${creator.actual_entries}`,
...
function renderInventorySummary(inventory: CorpusInventory): string {
  ...
  return [
    ...
    `entries with a source link: ${totals.entries_with_a_source_link}; with an incomplete metric list: ${totals.entries_with_incomplete_metrics}`,
