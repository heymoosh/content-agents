...
export function developSpawnPrompt(job: SpawnPromptJob): string {
  ...
  return enginePrompt(job.engine ?? "claude", "develop", [
    ...
    conversion?.ventureId ? `Use the Venture research questions to propose a small set of concrete tests across platforms, formats and source-supported angles. Open platform/audience questions are not evidence that LinkedIn or any other channel is best. Explain what each suggestion would help us learn, preserve an untreated comparison by default, and distinguish a broad exploratory batch from a controlled experiment. Do not select every platform-format-angle combination without making the workload clear. Existing publication on one platform is not a matched control for another platform.` : '',
