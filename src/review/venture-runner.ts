...
export function ventureStepPrompt(slug: string, phase: number, thread: unknown): string {
  return [
    ...
    "The server supplies the Venture slug to the CLI; never repeat the slug in args.",
    "Only `draft` and `thank-you-note-draft` take one argument: the safe lowercase artifact id they will draft. Every other allowed command takes `args: []`.",
    "For `plan-init`, input must use exactly: `confirmed_knowns: [{claim, evidence_refs, confirmed_by_muxin}]`, `open_unknowns: [{unknown_id, dimension, description, priority?}]`, and `probes: [{unknown_id, hypothesis, conversation_question, expected_evidence}]`. Use `confirmed_knowns: []` when no cited fact was explicitly confirmed by Muxin; do not substitute probe_id, targets_unknown_id, or evidence_that_would_move_it for the executable field names.",
