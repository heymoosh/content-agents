...
test("two Studio processes share one provider reconciliation lease", async () => {
  ...
  const ledgerPath = join(root, "events.jsonl"), healthPath = join(root, "health.json"), callsPath = join(root, "calls");
  writeFileSync(ledgerPath, JSON.stringify({ slug: "p", rowId: "r", provider: "typefully", state: "planned", at: "2026-01-01T00:00:00Z", providerObjectId: "tf-1" }) + "\n");
  const moduleUrl = pathToFileURL(join(process.cwd(), "src/review/provider-reconciliation-runner.ts")).href;
  const script = `import { appendFileSync } from "node:fs"; import { runProviderReconciliationOnce } from ${JSON.stringify(moduleUrl)}; await runProviderReconciliationOnce({ ledgerPath: process.argv[1], healthPath: process.argv[2], deps: { fetchTypefully: async () => { appendFileSync(process.argv[3], "call\\n"); await new Promise(r => setTimeout(r, 250)); return [{ id: "tf-1", whenIso: "2026-01-03T00:00:00Z", platforms: ["x"], title: "r" }]; } } });`;
  const run = promisify(execFile);
  await Promise.all([0, 1].map(() => run(process.execPath, ["--import", "tsx", "--input-type=module", "--eval", script, ledgerPath, healthPath, callsPath])));
