...
describe("assignedPairs: derived live from config/routing.yaml's defaults, never hardcoded", () => {
  ...
  test("against the LIVE config/routing.yaml: 22 assigned pairs across 6 pillars, including community targets", () => {
    const live = loadConfig();
    const pairs = assignedPairs(live, PILLARS_FIXTURE);
    assert.equal(pairs.length, 22);
    assert.deepEqual(pairs.filter((p) => p.platform === "x"), [
      { pillar: "claude-code", platform: "x" },
      { pillar: "builder", platform: "x" },
    ]);
