...
function insertPost(
  ...
  platform: string,
  fields: { source?: string | null; ctaDestination?: string | null },
  postedAt: string,
  likes: number
): void {
  const info = db
    .prepare(
      `INSERT INTO posts (platform, platform_post_id, posted_at, source, cta_destination) VALUES (?, ?, ?, ?, ?)`
    )
    .run(platform, `${platform}-${postedAt}-${Math.random()}`, postedAt, fields.source ?? null, fields.ctaDestination ?? null);
...
describe("LEVER_TRACKING_GAPS: fixed A/B/C entries, never computed", () => {
  test("exactly levers A, B, C are listed, in order, each with a card id and a non-empty reason", () => {
    assert.deepEqual(
...
describe("combineReport: composes Lever D (frame) + Lever E (cta) + the fixed A/B/C tracking gaps", () => {
  test("empty rows on both sides yield empty frame/cta arrays but the full tracking-gap list", () => {
    const report = combineReport([], [], cfg(), strategyCfg(), NOW);
    assert.deepEqual(report.frame, []);
    assert.deepEqual(report.cta, []);
    assert.equal(report.trackingGaps.length, 3);
  });

  test("a platform with sufficient frame data reads frame-winning, independent of cta data", () => {
  ...
  test("a platform with sufficient cta data reads clear-winner, independent of frame data", () => {
  ...
  test("both levers can report real reads in the same call, on different platforms", () => {
    ...
    const report = combineReport(frameRows, ctaRows, cfg(), strategyCfg(), NOW);
...
describe("buildReport: I/O wrapper reads from an injected DB via each lever's own loadRows", () => {
  test("frame + cta rows loaded from a shared injected DB compose into one report", () => {
    const db = freshDb();
  ...
  test("organic/exploration-only rows produce no frame or cta reads, only the tracking-gap section", () => {
    const db = freshDb();
    const d0 = new Date(NOW).toISOString();
...
describe("formatReport: pure string formatting, no I/O", () => {
  test("names all three tracking-gap levers by letter and includes each script name", () => {
    const report = combineReport([], [], cfg(), strategyCfg(), NOW);
    const text = formatReport(report, cfg());
    assert.match(text, /Insufficient tracking \(Levers A\/B\/C\)/);
    assert.match(text, /\| A \|/);
    assert.match(text, /\| B \|/);
    assert.match(text, /\| C \|/);
    assert.match(text, /npm run platform-fit/);
    assert.match(text, /npm run media-fit/);
    assert.match(text, /npm run cadence-fit/);
  });
  ...
  test("no frame or cta rows at all falls back to the explicit no-data guidance line, not a blank table", () => {
    const report = combineReport([], [], cfg(), strategyCfg(), NOW);
    const text = formatReport(report, cfg());
  ...
  test("never claims a lever-A/B/C lift number anywhere in the output", () => {
    const report = combineReport([], [], cfg(), strategyCfg(), NOW);
    const text = formatReport(report, cfg());
