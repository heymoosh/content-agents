...
export const destinationSchema = z.object({
  ...
  status: z.enum(['proposed', 'building', 'ready', 'retired']),
  url: z.string().max(2000).default(''),
}).refine(d => !d.url || safeDestinationUrl(d.url), 'Use a public HTTPS destination without credentials')
  .refine(d => d.status !== 'ready' || !!d.url, 'A ready destination needs its existing URL');
...
export function conversionDestinations(slug: string, plan = readConversionPlan(slug)): ConversionDestination[] {
  ...
  const derived = artifacts.map(a => {
    ...
    return { id: a.artifact_id, name: a.title, audience: 'See the Venture research plan', benefit: a.title,
  ...
  return [...plan.destinations.filter(d => !derived.some(a => a.id === d.id)), ...derived.map(a => {
    const metadata = plan.destinations.find(d => d.id === a.id);
    return metadata ? { ...metadata, status: a.status, url: a.url } : a;
  })];
}
