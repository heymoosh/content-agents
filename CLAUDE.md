...
Read and follow `AGENTS.md` for the repository operating policy. This file holds the product
architecture and safety rules that apply alongside it.
...
| Venture intake | `/venture new <slug>` | `npm run venture:new` | GUI: LLM maps founder notes and existing answers into the 25-field context, voice evidence, and Day 14 scorecard; asks only about gaps/ambiguity; owner reviews and confirms. CLI accepts completed context. | `venture/<slug>/intake.md`, notes provenance in `intake-context.json` |
